# SurveyMAE 项目完善方案：评测指标形式化与后续任务规划（v2）
...
| **SurveyMAE（本项目）** | **工具产出的结构化指标 + 检索增强的后验分析** | **基于证据的Agent判断** | **强：每条评分绑定工具证据 + 指标元数据标记LLM参与度** | **多模型投票+辩论 + 波动范围报告** |
...
> SurveyMAE提出"证据化评测"范式：将学术综述的质量评估分解为**可计算证据层**（引用图分析、检索增强的时序/覆盖分析、结构-聚类对齐等确定性工具输出）和**可解释判断层**（多角色Agent基于证据的rubric化评分），每个指标携带LLM参与度标记与波动范围，通过多模型投票与辩论机制提升评分鲁棒性，并以结构化JSON协议贯穿全流程实现评分可追溯。
...
```
┌─────────────────────────────────────────────────────────┐
│  第三层：综合诊断（Aggregator + ReporterAgent）           │
│  输入：各Agent的结构化评分JSON                            │
│  输出：加权总分 + 诊断报告 + 改进建议                     │
│  确定性指标用实线展示，LLM指标用误差棒展示波动范围        │
└────────────────────────┬────────────────────────────────┘
                         │
┌────────────────────────┴────────────────────────────────┐
│  第二层：多角色Agent判断（基于证据的rubric化评分）        │
│  VerifierAgent / ExpertAgent / ReaderAgent / Corrector   │
│  每个Agent输出结构化JSON，包含子维度分数+证据引用         │
│  所有LLM参与的指标标记 llm_involved=true + 波动范围      │
└────────────────────────┬────────────────────────────────┘
                         │
┌────────────────────────┴────────────────────────────────┐
│  第一层：确定性工具证据层（含检索增强）                   │
│  CitationChecker → CitationAnalyzer（含学术API检索）      │
│  → CitationGraph → LiteratureSearch（后验扩展检索）       │
│  输出：结构化统计数据（JSON），标记 llm_involved=false     │
└─────────────────────────────────────────────────────────┘
```
...
[CitationChecker.validate]
  ...
  │    引用列表/venue/关键词)       │  引用列表、venue、abstract关键词
  │                                │
  │   ┌────────────────────────────┘
...
**复用原则：** ref_metadata_cache是整个evidence_collection阶段的核心共享数据结构。CitationChecker在validate阶段获取的元数据应完整缓存，后续所有工具直接读取cache而非重新请求API。关键词提取也只执行一次，结果在field_trend_baseline检索和candidate_key_papers检索之间共享。
...
**输入证据：** C3, C5验证结果、抽样的引用-断言配对、missing_key_papers列表
...
| V2: 引用支持性 | true | 正文断言是否被所引文献支持 | 抽样的citation-sentence对 | 判断句子含义是否与引用内容匹配 |
| V3: 引用准确性 | true | 对引用文献的理解是否正确 | 抽样的citation-sentence对 | 检测误读、过度概括、错误归因 |
| V4: 内部一致性 | true | 综述内部是否存在自相矛盾 | 全文parsed_content | 检测前后矛盾的论断 |

**V2 Rubric（引用支持性）：**
- 5分：抽样的引用-断言对中 ≥90% 被充分支持
- 4分：70%-89% 被充分支持，少量部分支持
- 3分：50%-69% 被支持，存在个别无关引用
- 2分：30%-49% 被支持，多处引用与断言不匹配
- 1分：<30% 被支持，大量虚假/无关引用
...
#### 2.6.4 CorrectorAgent（偏差校正）

**输入证据：** 其他三个Agent的评分JSON

| 功能 | 描述 | 机制 |
|------|------|------|
| 多模型投票 | 对每个子维度，用3个不同厂商模型独立打分 | 取中位数作为修正分 |
| 异常值检测 | 标记模型间评分差异 > 2分的子维度 | 触发辩论或人工审查 |
| 偏差报告 | 输出各模型的评分分布与一致性指标 | 用于meta-evaluation |
| 波动范围计算 | 对每个 llm_involved=true 的指标计算 score_range 和 std | 填充指标元数据中的 reported_variance |
...
```json
{
  ...
  "sub_scores": {
    ...
    "V2_citation_supportiveness": {
      "score": 3,
      "llm_involved": true,
      "tool_evidence": {
        "metric": "sampled_pairs",
        "sample_size": 15,
        "supported_count": 9,
        "partial_count": 3,
        "unsupported_count": 3
      },
      "llm_reasoning": "Of 15 sampled citation-claim pairs, 9 are fully supported...",
      "flagged_items": [
        {"citation": "[12]", "sentence": "...", "issue": "overgeneralization"},
        {"citation": "[25]", "sentence": "...", "issue": "misattribution"}
      ],
      "variance": {
        "models_used": ["gpt-4o", "claude-sonnet-4-20250514", "deepseek-chat"],
        "scores": [3, 3, 2],
...
**Agent分发映射：**
```
VerifierAgent ← C3, C5完整数据 + 抽样citation-sentence对 + 指标定义
...
CorrectorAgent ← 上述三个Agent的输出JSON
```
...
[evidence_collection] ── 统一执行所有工具，产出结构化证据
  │  ├─ CitationChecker.validate ─→ validation (C3,C5) + ref_metadata_cache ★核心共享数据
...
[evidence_dispatch] ──── 组装Evidence Report（完整数据+定义+异常标记）
  │
  ├──→ [verifier_eval] ── VerifierAgent + 验证证据 → verifier_output.json
  ├──→ [expert_eval] ──── ExpertAgent + 图分析+缺失文献 → expert_output.json
  └──→ [reader_eval] ──── ReaderAgent + 时序+结构统计 → reader_output.json
  │
  ▼
[corrector_eval] ─────── CorrectorAgent: 多模型投票校正 + 波动范围计算
  │                       → corrector_output.json
  ▼
...
[debate] ──→│
  │         │
  ▼         ▼
[aggregator] ──────────── 加权聚合所有评分 → aggregated_scores.json
  │                       （确定性指标直接聚合，LLM指标带波动范围）
  ▼
...
**变化1：evidence_collection以ref_metadata_cache为核心共享数据。** CitationChecker.validate阶段获取的完整参考文献元数据（标题、年份、被引次数、引用列表、venue等）缓存后供所有后续工具复用，避免重复API调用。

**变化2：新增LLM辅助预处理（低幻觉风险）。** 关键词提取和候选列表清洗使用LLM辅助，标记为`hallucination_risk: low`，与第二层Agent的评判性使用做明确区分。关键词提取只执行一次，结果在T2/T5和G4之间共享。

**变化3：evidence_dispatch输出完整Evidence Report。** 不压缩数据，附带指标定义和异常标记。

**变化4：CorrectorAgent新增波动范围计算职责。** 对所有llm_involved=true的指标计算多模型评分的score_range和std，填入指标元数据。

**变化5：Reporter区分展示确定性指标和LLM指标。** 确定性指标用确定值展示，LLM指标带误差棒/波动范围展示。
...
class ToolEvidence(TypedDict):
    extraction: dict       # citation extraction结果
    validation: dict       # metadata验证结果 (C3, C5)
    analysis: dict         # 时序/结构统计 (T1-T5, S1-S5)
    graph_analysis: dict   # 引用图分析 (G1-G6)
...
class MetricMetadata(TypedDict):
    ...
    variance_strategy: Optional[dict]
    reported_variance: Optional[dict]
```
...
标注者2-3位，使用与系统相同的rubric打1-5分。标注流程含pilot校准+独立标注+一致性计算（Krippendorff's alpha）+共识讨论。最小标注量：10篇 × 12子维度 × 2标注者 = 240个评分点。
...
| A3: 无多模型投票 | CorrectorAgent只用单模型 | 多模型投票对稳定性的贡献 |
| A4: 无辩论 | 关闭debate机制 | 辩论对评分收敛的贡献 |
...
- [ ] 1.4 实现指标元数据schema（MetricMetadata），所有指标携带llm_involved标记
- [ ] 1.5 完善所有子维度的rubric（参照本文档§2.6模板）
- [ ] 1.6 设计Agent输出JSON Schema（参照§2.7）
- [ ] 1.7 实现evidence_dispatch逻辑：组装Evidence Report（完整数据+定义+异常标记）
- [ ] 1.8 更新各Agent的system prompt模板，嵌入rubric、输出格式要求和指标定义
- [ ] 1.9 更新state.py添加ToolEvidence、MetricMetadata等字段

### Phase 2（高优先级，1-2周）：Workflow重构 + ReporterAgent分离

- [ ] 2.1 实现evidence_collection节点（统一工具执行 + 检索增强步骤）
- [ ] 2.2 实现evidence_dispatch节点（证据分发 + Evidence Report组装）
- [ ] 2.3 拆分ReporterAgent为Aggregator（纯计算）+ Reporter（报告生成）
- [ ] 2.4 Reporter实现区分展示：确定性指标实线 + LLM指标带误差棒
- [ ] 2.5 CorrectorAgent实现波动范围计算
- [ ] 2.6 更新builder.py的节点连接
- [ ] 2.7 端到端集成测试
...
- [ ] 3.3 收集人类标注
- [ ] 3.4 运行系统评测，收集输出（含波动范围数据）
- [ ] 3.5 计算一致性指标，执行消融实验（含A2检索增强消融）
- [ ] 3.6 分析确定性指标 vs LLM指标的贡献差异

### Phase 4（后续，1-2周）：论文撰写 + 可视化

- [ ] 4.1 撰写论文
- [ ] 4.2 前端可视化：引用图、评分雷达图（确定性/LLM指标区分）、诊断报告
- [ ] 4.3 代码整理、文档完善
...
2. **S5的聚类稳定性。** 引用图聚类算法对参数敏感，不同运行可能产生不同聚类结果。建议固定随机种子并报告聚类算法与参数选择的影响。

3. **G4中top-K阈值的选择。** K太小可能遗漏重要文献，K太大会引入噪声（边缘论文也被当作"核心"）。建议在实验中测试K=20/30/50的敏感性。

4. **LLM参与指标的波动范围。** 如果某个子维度在3个模型间的评分差异 > 2分，说明该维度的rubric可能不够精确，需要细化。这应该在pilot阶段就发现并修正。

5. **不要高估辩论机制的价值。** 在消融实验中严格验证，如果辩论不带来统计显著改善，可以简化为单轮异常值检测。
