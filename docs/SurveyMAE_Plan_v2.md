...
**批处理机制：** 为控制成本和延迟，C6采用批处理方式调用LLM——每次prompt中构造`batch_size`个sentence+abstract对（默认10，可在config中调整），LLM对每对输出`support`/`contradict`/`insufficient`三分类。使用轻量模型（如qwen3.5-flash）即可，支持并发批处理。
...
```json
{
  ...
  "sub_scores": {
    ...
    "V2_citation_claim_alignment": {
      ...
      "llm_reasoning": "C6 reports 1.6% contradiction rate (5/312 pairs). Upon review: 3 are genuine mischaracterizations of cited methods, 2 are false positives where the abstract omits relevant details discussed in the full paper.",
      "flagged_items": [
        {"citation": "[68]", "sentence": "MolReGPT leverages RAG to enhance...", "issue": "method mischaracterization", "confirmed": true},
        {"citation": "[20]", "sentence": "...", "issue": "scope overgeneralization", "confirmed": true}
      ],
      "variance": {
        "models_used": ["gpt-4o", "claude-sonnet-4-20250514", "deepseek-chat"],
        "scores": [4, 4, 3],
...
[evidence_dispatch] ──── 组装Evidence Report（完整数据+定义+异常标记）
  ...
  │
  ├──→ [verifier_eval] ── VerifierAgent(V1+V2条件+V4) → verifier_output.json
...
| A4: 无多模型投票 | CorrectorAgent只用单模型 | 多模型投票对稳定性的贡献 |
| A5: 无辩论 | 关闭debate机制 | 辩论对评分收敛的贡献 |
| A6: 单Agent | 合并为一个Agent做全维度评估 | 角色分工对评分质量的贡献 |
...
- [ ] 1.5 完善所有子维度的rubric（参照本文档§2.6模板，注意V2/V3已合并）
- [ ] 1.6 设计Agent输出JSON Schema（参照§2.7）
- [ ] 1.7 实现evidence_dispatch逻辑：组装Evidence Report（完整数据+定义+异常标记）+ C6.auto_fail短路预填V2=1分
- [ ] 1.8 更新各Agent的system prompt模板，嵌入rubric、输出格式要求和指标定义
- [ ] 1.9 更新state.py添加ToolEvidence、MetricMetadata等字段
- [ ] 1.10 实现C6 citation_sentence_alignment：批处理LLM调用 + contradiction_rate计算 + auto_fail短路逻辑 + citation_alignment.yaml prompt模板
